...
class NetworkTrainer:
    ...
    def call_dit(
        ...
        if args.gradient_checkpointing:
            ...
            print("Checkpoint gradient is active")
    ...
    def train(self, args):
        ...
        if args.gradient_checkpointing:
            print("Accelerator gradient checkpointing is true")
        ...
        if accelerator.is_main_process: # Solo imprimir en el proceso principal
            #print("--- Verificando parámetros entrenables de la red LoRA (después de accelerator.prepare) ---")
            # Es importante unwrappear el modelo si fue wrappeado por DDP u otra estrategia de accelerate
            unwrapped_network_for_check = accelerator.unwrap_model(network) 
            found_any_trainable_in_lora = False
            for name, param in unwrapped_network_for_check.named_parameters():
                if param.requires_grad:
                    print(f"    Trainable en LoRA: {name} - Shape: {param.shape} - Device: {param.device} - Requires Grad: {param.requires_grad}")
                    found_any_trainable_in_lora = True
            
            if not found_any_trainable_in_lora:
                print("    ALERTA CRÍTICA: ¡No se encontraron parámetros con requires_grad=True en la red LoRA después de accelerator.prepare!")
            else:
                print("    OK: Lora blocks parameters found.")
            print("--- STARTING RUN ---")
